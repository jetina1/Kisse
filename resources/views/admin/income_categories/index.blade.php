                            ...
                            <tbody>
                                @forelse($income_categories as $income_category)
                                    <tr>
                                        ...
                                        <td><a
                                                href="{{ route('admin.income_categories.show', $income_category) }}">{{ $income_category->name }}</a>
                                        </td>
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="{{ route('admin.income_categories.edit', $income_category) }}"
                                                    class="btn btn-sm btn-primary">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                                <form onclick="return confirm('are you sure !')"
                                                    action="{{ route('admin.income_categories.destroy', $income_category) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
